...
const ChartsPage = () => {
    ...
    const sortedByAccessTime = [...regions].sort((a, b) => b.avgAccessTime - a.avgAccessTime).slice(0, 10);
    ...
    const highAgingHighAccessTime = regions.filter(r => r.agingRate > 30 && r.avgAccessTime > 100).length;
    const analysisText1 = `고령화율이 30% 이상인 지역 중 ${highAgingHighAccessTime}곳이 평균 접근 시간이 100분을 초과하여 교통 취약도가 매우 높게 나타납니다.`;

    return (
        <div className="p-4 pb-20">
            <h1 className="text-3xl font-bold text-gray-800 mb-6">데이터 분석</h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                >
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={sortedByAccessTime} layout="vertical" margin={{ top: 5, right: 30, left: 40, bottom: 5 }}>
                            <CartesianGrid strokeDasharray="3 3" />
                            <XAxis type="number" />
                            <YAxis dataKey="regionName" type="category" width={100} style={{ fontSize: '12px' }} />
                            <Tooltip />
                            <Legend />
                            <Bar dataKey="avgAccessTime" fill="#ef4444" name="평균 접근 시간 (분)" />
                        </BarChart>
                    </ResponsiveContainer>
                </ChartCard>
